...
class Tx_PtExtbase_State_Session_Storage_SessionAdapter implements Tx_PtExtbase_State_Session_Storage_AdapterInterface {
	...
	public function read($key, $allowUnserializing = true) {
		...
		if (TYPO3_MODE == 'FE' && ($GLOBALS['TSFE']->fe_user instanceof \TYPO3\CMS\Frontend\Authentication\FrontendUserAuthentication)) {
			...
			if (TYPO3_DLOG) GeneralUtility::devLog(sprintf('Reading "%s" from FE browser session in "$GLOBALS[\'TSFE\']->fe_user"', $key), 'pt_extbase');
	...
	public function store($key, $val, $allowSerializing = true, $foreignSessionId = NULL) {
		...
		if (TYPO3_MODE == 'FE' && ($GLOBALS['TSFE']->fe_user instanceof \TYPO3\CMS\Frontend\Authentication\FrontendUserAuthentication)) {
			...
			if (is_null($foreignSessionId)) {
				...
				if (TYPO3_DLOG) GeneralUtility::devLog(sprintf('Storing "%s" into FE browser session using "$GLOBALS[\'TSFE\']->fe_user"', $key), 'pt_extbase');
			} else {
				...
				if (TYPO3_DLOG) GeneralUtility::devLog(sprintf('Storing "%s" into foreign FE browser session "%s"', $key, $foreignSessionId), 'pt_extbase');
			}
	...
	public function delete($key) {
		...
		if (TYPO3_MODE == 'FE' && ($GLOBALS['TSFE']->fe_user instanceof \TYPO3\CMS\Frontend\Authentication\FrontendUserAuthentication)) {
			...
			if (!empty($GLOBALS['TSFE']->fe_user->sesData[$key])) {
				...
				$GLOBALS['TSFE']->fe_user->sesData_change = 1;
				$GLOBALS['TSFE']->fe_user->storeSessionData();
				if (TYPO3_DLOG) GeneralUtility::devLog(sprintf('Deleting "%s" from FE browser session in "$GLOBALS[\'TSFE\']->fe_user"', $key), 'pt_extbase');
			}
			...
			if (!empty($sesDat[$key])) {
				...
				if (TYPO3_DLOG) GeneralUtility::devLog(sprintf('Deleting "%s" from BE user in "$GLOBALS[\'BE_USER\']"', $key), 'pt_extbase');
			}
